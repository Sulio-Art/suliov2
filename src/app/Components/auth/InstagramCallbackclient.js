...
function InstagramCallbackHandler() {
  ...
  useEffect(() => {
    ...
    if (errorDescription) {
      ...
      setError(`Instagram login failed: ${friendlyError}`);
    ...
    if (code) {
      signIn("credentials", {
        ...
        if (result.ok && !result.error) {
          toast.success(
            "Successfully connected with Instagram! Redirecting to dashboard..."
          );
          router.push("/dashboard");
        } else {
          setError(result.error || "An unknown error occurred.");
          toast.error(result.error || "Could not log in with Instagram.");
        }
      });
    } else {
...
function InstagramCallbackHandler() {
      ...
      signIn('credentials', {
        ...
        if (result.ok) {
          ...
          try {
            ...
            if (result.status === 201 && errorResponse.completionToken) {
              toast.success("Welcome! Let's complete your profile.");
              

              sessionStorage.setItem('igCompletionToken', errorResponse.completionToken);
              sessionStorage.setItem('igPrefillData', JSON.stringify(errorResponse.prefill));
  ...
  if (!isLoading && error) {
    return (
      <div className="text-center p-8 bg-white shadow-md rounded-lg max-w-md">
        ...
        <p className="text-gray-700">{error}</p>
        <button onClick={() => router.push('/')} className="mt-6 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
          Return to Homepage
        </button>
      </div>
    );
  ...
  return (
    <div className="flex items-center space-x-3">
      <Loader2 className="h-8 w-8 animate-spin text-blue-600" />
      <p className="text-xl text-gray-700">Finalizing connection, please wait...</p>
    </div>
  );
}
